...
export const singleValueArb = (): unknown =>
...
export const plusP = curry((x: number, y: number) =>
  Promise.resolve(plus(x, y)),
);

export const minus = curry((x: number, y: number) => x - y);

export const minusP = curry((x: number, y: number) =>
  Promise.resolve(minus(x, y)),
);

export const sum = (...xs: number[]): number =>
  xs.reduce((memo, a) => memo + a, 0);
