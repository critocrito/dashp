...
testProp("composition of applicative", [fc.nat(), fc.nat(), fc.nat()], async (t, a, b, c) => {
  const x = of(a);
  const u = of(plus(b));
  const v = of(plus(c));
...
testProp("applicative deriving Functors map", [fc.nat(), fc.nat()], async (t, a, b) => {
  const f = plus(b);
...
testProp("monad associativity", [fc.nat(), fc.nat(), fc.nat()], async (t, a, b, c) => {
  const f = plusP(b);
  const g = plusP(c);

  return t.is(await chain(f, chain(g, of(a))), await chain((x: number) => chain(g, f(x)), of(a)));
});
