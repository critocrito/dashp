...
export const unlessElse = curry4(
  promisify((predicate, consequent, alternative, value) => {
    ...
    return whenElse(predicate, alternative, consequent, value);
  })
);
...
export const when = curry3(
  promisify((predicate, consequent, value) => {
    ...
    return whenElse(predicate, consequent, identity, value);
  })
);
...
export const unless = curry3(
  promisify((predicate, consequent, value) => {
    ...
    if (!isFunction(consequent))
      invalidFunction("Future#unless", 1, consequent);
    return unlessElse(predicate, consequent, identity, value);
  })
);
