...
export const whenElse = curry4("whenElse", (predicate, consequent, alternative, p) => {
  const branch = (x) =>
    of(predicate(x)).then((bool) => (bool === true ? consequent(x) : alternative(x)));
  return of(p).then(branch);
});

export const unlessElse = curry4("unlessElse", (predicate, consequent, alternative, p) =>
